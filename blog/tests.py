...
class TestView(TestCase):
    ...
    def category_card_test(self, soup):
        ...
        self.assertIn('카테고리', categories_card.text)
    ...
    def test_comment_form(self):
        ...
        # 로그인 한 상태
        # self.client.login(username='obama', password='passowrd')
        self.client.login(username=self.user_obama.username, password='password')
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        response = self.client.post(self.post_001.get_absolute_url() + 'new_comment/', {'content':'오바마의 댓글입니다.',}, follow=True)
    ...
    def test_comment_update(self):
        commnet_by_trump = Comment.objects.create(
            post = self.post_001,
        ...
        response = self.client.post(f'/blog/update_comment/{self.comment_001.pk}/', 
                                        {
